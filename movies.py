...
# PEP-8 jak se má formátovat v pythonu
...
if not uzivatel in uzivatele:
  print("Nejsi registrován, gtfo.")
  quit()
else:
  print("V pořádku", oddelovac, sep="\n")
  print("Vitejte v našem filmovém slovníku!".upper().center(62),
        oddelovac, sep="\n")
...
# konec else:  print("Vybrana sluzba neni v nabidce, ukoncuji..", oddelovac, sep="\n")
  ...
  spolecny_prvni_uziv = aktivni_uzivatel.intersection(hodnoceni_prvni_uz)
  spolecny_druhy_uziv = aktivni_uzivatel.intersection(hodnoceni_druhy_uz)

  if len(spolecny_prvni_uziv) > len(spolecny_druhy_uziv):
    nejvice_spolecnych = uzivatele[jmeno_prvni_uz]

  elif len(spolecny_prvni_uziv) < len(spolecny_druhy_uziv):
